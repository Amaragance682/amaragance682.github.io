...
window.onload = function init()
{
    ...
    function rotateX(angle) {
        return mat4(
            1, 0, 0, 0,
            0, Math.cos(radians(angle)), -Math.sin(radians(angle)), 0,
            0, Math.sin(radians(angle)), Math.cos(radians(angle)), 0,
            0, 0, 0, 1
        );
    }
    ...
    function rotateY(angle) {
        return mat4(
            Math.cos(radians(angle)), 0, Math.sin(radians(angle)), 0,
            0, 1, 0, 0,
            -Math.sin(radians(angle)), 0, Math.cos(radians(angle)), 0,
            0, 0, 0, 1
        );
    }
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 57:	
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Horfa alltaf á eitt hús innan úr bílnum";
                break;
...
function render()
{
    ...
    switch( view ) {
        ...
        mv = mult(mv, rotateX(rotationX));
        mv = mult(mv, rotateY(rotationY));
	...
	case 8:
	    ...
	    break;
    case 9:
        // Drive around while looking at a house at (-40, 140)
	    mv = rotateY( -carDirection );
        mv = lookAt( vec3(125.0, 0.0, 5.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
    ...
    canvas.addEventListener("mousemove", function(e) {
        if (movement) {
            var deltaX = e.offsetX - origX;
            var deltaY = e.offsetY - origY;
